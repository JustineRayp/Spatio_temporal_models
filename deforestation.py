...
def land_constraint_unrestricted(i, j, land_use_dict):
    if land_use_dict['waterways'][i, j] == 0:
        return 0
    return 1
...
def land_constraint_policy_driven(i, j, land_use_dict, protection_percentage):
    if land_use_dict['natural'][i, j] == 0:
...
def become_forest_probability(i, j, array, neighbor_threshold=5, growth_threshold=0.3):
...
def land_constraint_partial_protection_with_growth(i, j, land_use_dict, protection_percentage, growth_threshold=0.3):
    ...
    if land_use_dict['natural'][i, j] == 1 and land_use_dict['waterways'][i, j] == 1: #allow non-urbanized cells that aren't water to become forest again
        if become_forest_probability(i, j, land_use_dict['natural'], growth_threshold):
            #land_use_dict['natural'][i, j] = 0
            return 0
    return 1
...
def run_scenario(data_dict, proxy_dict, land_use_dict, first_year, last_year, coefficients, intercept, land_constraint_func, *args):
    ...
    developed_cells = 0
    land_use_dict_2 = copy.deepcopy(land_use_dict)
    ...
    for score, (i, j) in evaluation_list:
        ...
        if land_constraint_func(i, j, land_use_dict, *args): #landuse_constraint_func returns 1 if cell can be developed, 0 if it can't
            ...
            developed_cells += 1
        if land_constraint_func(i, j, land_use_dict, *args) == 0:
            predicted_array[i,j] = 0
    
    oa = calculate_overall_accuracy(predicted_array, end_array)
    fom = calculate_figure_of_merit(predicted_array, end_array)

    return predicted_array, oa, fom, land_use_dict_2


# Code to run a certain scenario, based on the original model
def run_scenario_reforest(data_dict, proxy_dict, land_use_dict, first_year, last_year, coefficients, intercept, land_constraint_func, *args):
    first_value = 'data_' + str(first_year)
    last_value = 'data_' + str(last_year)
    ...
    for score, (i, j) in evaluation_list:
        ...
        if land_constraint_func(i, j, land_use_dict, *args) == 0:
            predicted_array[i,j] = 0
            if land_use_dict['natural'][i, j] == 1 and land_use_dict['waterways'][i, j] == 1: #in case of reforestation
                land_use_dict_2['natural'][i, j] = 0
...
def evaluate_deforestation_rate(start_array, predicted_array, land_use_dict, land_use_dict_2):
    forest_cells_start = np.sum((start_array == 0) & (land_use_dict['natural'] == 0))
    forest_cells_end = np.sum((predicted_array == 0) & (land_use_dict_2['natural'] == 0))
...
def visualize_results(start_array, predicted_array, land_use_dict, land_use_dict_2, title):
    ...
    for i in range(start_array.shape[0]):
        for j in range(start_array.shape[1]):
            ...
            elif land_use_dict['waterways'][i, j] == 0 & start_array[i,j] == 0:
                color_map_start[i, j] = water_color
            elif land_use_dict['natural'][i, j] == 0 & start_array[i, j] == 0:
    ...
    for i in range(predicted_array.shape[0]):
        for j in range(predicted_array.shape[1]):
            ...
            elif land_use_dict_2['waterways'][i, j] == 0 & predicted_array[i,j] == 0:
                color_map_predicted[i, j] = water_color
            elif land_use_dict_2['natural'][i, j] == 0 & predicted_array[i,j] == 0 :
...
def run_all_scenarios():
    ...
    predicted_array, oa, fom, landuse_dict_2  = run_scenario(data_dict, feature_dict, landuse_dict, first_year, last_year, coefficients, intercept, land_constraint_unrestricted)
    deforestation_rate = evaluate_deforestation_rate(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2)
    visualize_results(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2, f'Unrestricted Urbanization\nOA: {oa:.4f}, FoM: {fom:.4f}, Deforestation Rate: {deforestation_rate:.4f}')
    ...
    for protection_percentage in protection_percentages:
        predicted_array, oa, fom, landuse_dict_2 = run_scenario(data_dict, feature_dict, landuse_dict, first_year, last_year, coefficients, intercept, land_constraint_policy_driven, protection_percentage)
        deforestation_rate = evaluate_deforestation_rate(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2)
        visualize_results(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2, f'Policy-Driven Forest Protection ({int(protection_percentage * 100)}%)\nOA: {oa:.4f}, FoM: {fom:.4f}, Deforestation Rate: {deforestation_rate:.4f}')

    # Scenario 3: Partial Forest Protection with Development Constraints and Growth
    for protection_percentage in protection_percentages:
        predicted_array, oa, fom, landuse_dict_2 = run_scenario_reforest(data_dict, feature_dict, landuse_dict, first_year, last_year, coefficients, intercept, land_constraint_partial_protection_with_growth, protection_percentage)
        deforestation_rate = evaluate_deforestation_rate(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2)
        visualize_results(data_dict['data_' + str(first_year)], predicted_array, landuse_dict, landuse_dict_2, f'Partial Forest Protection with Growth ({int(protection_percentage * 100)}%)\nOA: {oa:.4f}, FoM: {fom:.4f}, Deforestation Rate: {deforestation_rate:.4f}')
